-- v2: CaixaNow y el fichero SEPA ordenan distinto los movimientos intradía, así que
-- el saldo puede no coincidir para el mismo movimiento. Fase 1: match exacto
-- fecha+importe+saldo. Fase 2: los restantes se emparejan por rango de saldo dentro
-- de cada grupo (fecha, importe). Inserción por diferencia de cardinalidad.
-- NOTA: la inserción de esta versión contaba la cardinalidad solo sobre las filas
-- no coincidentes por saldo; corregida en la v3 (20260831084357), que redefine la
-- función entera. Se conserva tal cual se aplicó.
...
begin
  ...
  with grupos_db as (
    ...
    select s.*, coalesce(g.dn, 0) as dn,
           row_number() over (partition by s.fecha, s.importe order by s.saldo) as rn,
           count(*) over (partition by s.fecha, s.importe) as sn
    from fin_n43_staging s
    left join grupos_db g on g.fecha = s.fecha and g.importe = s.importe
    where s.fecha >= date '2026-01-01'
      and (g.saldos is null or not s.saldo = any(g.saldos))
  )
