...
- Remesas (F4a, APLICADA 16-08, registrada como 20260816205218): fin_bancos_cuentas, fin_mandatos, fin_remesas y fin_remesas_items, mas identificador_acreedor en fin_config. Los ficheros los construye lib/sepa.ts (pain.001 para pagos, pain.008 para cobros), que es funcion pura y tiene 24 pruebas. Una remesa generada no se puede tocar: se anula y se rehace.

- Diario e informes (F5a, APLICADA 17-08-2026, registrada como 20260817112052): fin_asientos y fin_apuntes existian desde la F0 pero sin nada que las gobernara — la propia F0 dejo anotado que el cuadre lo garantizaria «la funcion de contabilizacion», y esa funcion no se habia escrito nunca. La F5a la escribe: un asiento nace borrador y sin numero (habia que cambiar el default de `estado`, que era 'confirmado'), fin_confirmar_asiento() comprueba cuadre, ejercicio abierto, mes no bloqueado y cuentas de la sociedad, numera por ejercicio y cierra; un asiento confirmado y sus apuntes son inmutables. Probada contra la base real con rollback, incluido el borrado en cascada que motivo la F0b. Revisada dos veces el 17-08 tras sendos repasos del chat: (1) cerrar el UPDATE directo que confirmaba sin pasar por la funcion — la RLS da UPDATE completo, asi que cerrar solo el INSERT no bastaba, el mismo error que la F2b corrigio en facturas; (2) bloquear la cabecera tambien al BORRAR apuntes, no solo al insertarlos, porque la carrera es simetrica; y `found` en vez de una variable propia, porque `select true, … into v_existe` deja NULL y no false cuando no hay fila, lo que convertia esa comprobacion en codigo muerto.
- Tercer repaso (17-08): el disparador de apuntes pasa a SECURITY DEFINER. Un disparador que no lo es corre bajo la RLS del usuario, asi que `not found` significaba «no lo veo», no «no existe» — y como la politica de fin_apuntes filtra por el cuenta_id DEL APUNTE, que no estaba atado al de su asiento, la inmutabilidad se apagaba sola para una pareja descabalada. Ahora ademas se exige que apunte y asiento sean de la misma cuenta. Y se bloquean con `for share` el ejercicio y el periodo al confirmar: cerrar un ejercicio o bloquear un mes son hoy updates directos, asi que sin lock se podia confirmar dentro de un ejercicio que acababa de cerrarse.
- Analitica por centro: `centro_id` va en el APUNTE (fin_apuntes), no en el asiento, porque un mismo asiento reparte una compra entre varios centros. Solo se ofrece y se guarda en cuentas de los grupos 6 y 7: una cuenta de tesoreria o de proveedores no tiene centro que valga. Es opcional a proposito -- los gastos de estructura no llevan centro --, y por eso la PyG filtrada por un centro NO suma el resultado de la empresa: lo que queda sin centro se cae del filtro. El balance no admite filtro por centro y es intencionado: filtrar un balance por centro no significa nada.
- La marca `fin.confirmando` es un GUARDARRAIL, no una barrera: set_config() lo puede llamar cualquiera. Por PostgREST no es alcanzable (vive en pg_catalog y no se expone como RPC) y eso es lo que hoy la sostiene. La barrera de verdad son los privilegios de columna de la seccion 5 de la F5a, ya aplicados: authenticated solo puede escribir ejercicio_id, fecha, descripcion, origen_tipo y origen_id.
- Periodos (F5b, SIN aplicar): disparador que siembra los doce periodos al dar de alta un ejercicio, mas recuperacion de los que falten. Es la primera capa; la segunda es que la F5a rechaza confirmar si falta la fila del mes, en vez de tratarla como mes abierto. Encima van tres informes (sumas y saldos, balance de situacion y perdidas y ganancias) con selector de mes, trimestre, acumulado anual o rango libre. El calculo esta en lib/contabilidad.ts, que es funcion pura y tiene 52 pruebas.
- OJO con los informes: el BALANCE es una foto acumulada a la fecha de corte y la PYG es el movimiento del tramo. Por eso datos.ts pide siempre los apuntes desde el 1 de enero y deja que sumasYSaldos separe «anterior» de «periodo». Pedir solo el tramo daria un balance que cuadra y esta mal.
- Informes rapidos (F5c, APLICADA 17-08-2026, registrada como 20260817195013): funcion fin_informe_mensual(anio) — agregado por (cuenta, mes, centro) para balance, PyG y sumas y saldos, security INVOKER (la RLS del que llama aplica). Con el diario real los informes no podian traerse 21.000 apuntes por visita: en serie agotaban la funcion de Vercel y en paralelo el limite por consulta de la base. La app usa la funcion cuando el periodo cae en meses enteros y cae al detalle paginado (max 3 consultas a la vez) solo en «entre dos fechas».
- Activos (F2c, APLICADA 16-08, registrada como 20260816211204): fin_activos y fin_amortizaciones, mas 23 cuentas del PGC sembradas para inmovilizado. El cuadro se calcula en lib/amortizacion.ts (lineal, mensual, primer mes prorrateado por dias, ultimo periodo ajustado para que el acumulado cuadre) y se guarda entero: un periodo contabilizado ya no se recalcula. Todavia NO genera asiento contable.
...
- ~~Lo que hay en fin_plan_cuentas NO es un plan de cuentas~~ RESUELTO el 17-08-2026: cargado el plan COMPLETO de A3 (3.166 cuentas, todas de 9 digitos, origen 'a3') desde el Excel de Lucia, via MCP en lotes idempotentes con cuadre verificado grupo a grupo contra el fichero. Las 23 pgc cortas de la F2c se retiraron (nada las referenciaba; fin_activos estaba a cero). cuentasSugeridas() habla ahora en 9 digitos y conserva el sufijo de centro (213000700 -> 281300700 y 681000700). El parrafo antiguo, para contexto: **Lo que habia en fin_plan_cuentas NO era un plan de cuentas.** Son 635 subcuentas de PROVEEDOR migradas de compras_cuenta_a3 (400000001 COCA-COLA...), todas del grupo 4. No hay cuentas de inmovilizado, ni de gasto, ni de ingreso, ni de tesoreria: hoy no se puede contabilizar nada. La F2c siembra las 23 del PGC que necesita el modulo de activos, marcadas con origen 'pgc' (23 y no 24: no existe la 2810, porque su contrapartida seria la 210 de terrenos y los terrenos no se amortizan). El plan de verdad esta en A3 y hay que exportarlo de alli para que coincida con el de la asesoria. Luis lo pide a Lucia (17-08-2026), junto con proveedores y clientes completos, facturas de gasto y cierres diarios de Agora.
- **Los informes no son el modelo oficial de cuentas anuales.** El balance y la PyG del PGC llevan epigrafes normalizados (A-1, B-I…) y para emitirlos hace falta un mapa cuenta→epigrafe que decide la asesoria. Lo que hay es el balance y la PyG DE GESTION: mismas cifras, agrupadas por masas patrimoniales y por subgrupo. Para mirar el negocio sirve; para depositar en el registro, no.
- **Una cuenta que no se sabe clasificar NO se coloca a ojo.** En lib/contabilidad.ts cae en «sin clasificar» y sale a la vista en el balance, con su descuadre. Es a proposito: el grupo 4 y el 5 mezclan activo y pasivo (472 es un derecho y 477 una deuda, y las dos empiezan por 47), y una cuenta metida en la masa equivocada cuadra igual y no la ve nadie nunca.
