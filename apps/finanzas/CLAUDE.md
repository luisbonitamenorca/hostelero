...
- La marca `fin.confirmando` es un GUARDARRAIL, no una barrera: set_config() lo puede llamar cualquiera. Por PostgREST no es alcanzable (vive en pg_catalog y no se expone como RPC) y eso es lo que hoy la sostiene. La barrera de verdad serian privilegios de columna sobre fin_asientos. Decision pendiente de Luis.
